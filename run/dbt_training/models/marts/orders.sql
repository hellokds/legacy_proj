      ...
      create or replace transient table ANALYTICS.marts.orders  as
      (with

orders as (

    select * from ANALYTICS.staging.stg_tech_store__orders

),

transactions as (

    select * from ANALYTICS.staging.stg_payment_app__transactions

),

products as (

    select * from ANALYTICS.staging.stg_tech_store__products
